...
import atlasLogo from '../assets/images/atlas_logo_1784081371248.png';
...
export default function Navbar({ currentView, onViewChange, onOpenInquiry }: NavbarProps) {
  ...
  const [logoSrc, setLogoSrc] = useState<string>(atlasLogo || '/atlas_logo_1784081371248.png');
  const [hasImageError, setHasImageError] = useState(false);

  const handleLogoError = () => {
    if (logoSrc === atlasLogo) {
      setLogoSrc('/atlas_logo_1784081371248.png');
    } else if (logoSrc === '/atlas_logo_1784081371248.png') {
      setLogoSrc('/logo.png');
          ...
          >
            <div className="h-20 sm:h-24 md:h-26 w-auto flex items-center justify-start relative">
              {!hasImageError ? (
                ...
                <div className="flex items-center gap-2 px-3 py-1.5 rounded border border-accent-gold/40 bg-black/60 text-accent-gold font-display font-black text-xl tracking-wider">
                  ATLAS <span className="text-text-primary text-sm font-light">MEP GROUP</span>
                </div>
              )}
            </div>
          </div>
          ...
          >
            <div className="h-20 sm:h-24 w-auto flex items-center justify-center relative">
              {!hasImageError ? (
                ...
                <div className="flex items-center gap-1 px-2.5 py-1 rounded border border-accent-gold/40 bg-black/60 text-accent-gold font-display font-bold text-base tracking-wider">
                  ATLAS <span className="text-text-primary text-xs font-light">MEP</span>
                </div>
              )}
            </div>
          </div>
